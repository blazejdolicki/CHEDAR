    ...
    for epoch_number_id in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if qid.split('_')[-1] != '1':            
                ...
                #logger.info("Emb tensor shape: "+str(len(embs_list)))
                #logger.info("teacher_embs_list tensor shape: "+str(teacher_embs_list.shape))
                continue
            else:
                if len(embs_list) == 0:#first iteration of the loop is empty, we dont want to run backwards till the next conversation
                  with torch.no_grad(): #CHEDAR: dont train convdr model
                    ...
                    teacher_embs_list = [teacher_embs]
                    continue
                history_emb = history_encoder.init_hidden()
                embs_list = torch.stack((embs_list))                
                teacher_embs_list = torch.stack((teacher_embs_list))
                
                #logger.info("Emb tensor shape: "+str(embs_list.shape))
                #logger.info("teacher_embs_list tensor shape: "+str(teacher_embs_list.shape))
